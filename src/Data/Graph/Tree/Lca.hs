...
type LcaCache = (Permutation, U.Vector Int, VecBL Permutation)
...
foldLcaCache (!toParent, !_, !_) !edgeValueOf = cacheBL . TransiteSemigroup $ U.generate (G.length (unPermutation toParent)) f
  where
    f c
      | p == -1 = (-1, mempty)
      | otherwise =
...
foldViaLca :: forall a. (HasCallStack, U.Unbox a, Semigroup a) => LcaCache -> VecBL (TransiteSemigroup a) -> (Vertex, a) -> (Vertex, a) -> a
foldViaLca cache@(!_, !depths, !_) toParentBL (!v1, !a1) (!v2, !a2) = a'
  where
    (!_, !d) = lca cache v1 v2
    d1 = depths U.! v1
    d2 = depths U.! v2
