...
-- only.
scc1SG :: forall w m. (PrimMonad m) => SparseGraph w -> UM.MVector (PrimState m) Bool -> Vertex -> m [Vertex]
scc1SG !gr' !vis !v0 = do
...
sccSG :: (U.Unbox w) => SparseGraph w -> [[Int]]
sccSG gr = collectSccPreorderSG $ topSortSG gr
  where
    !gr' = revSG gr
