...
{-# INLINE newLazySS #-}
newLazySS :: forall v a m. (U.Unbox v, U.Unbox a, SegmentAction a v, PrimMonad m) => Int -> m (SplaySeq (PrimState m) v a)
newLazySS !n = do
  rawSS <- newLazyRSS n
  rootSS <- UM.replicate 1 (-1 :: SplayIndex)
  return SplaySeq {..}
...
{-# INLINE readSS #-}
readSS :: (HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> Int -> m v
...
{-# INLINE writeSS #-}
writeSS :: (HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> Int -> v -> m ()
writeSS SplaySeq {..} k v = do
  GM.unsafeModifyM rootSS (writeRSS rawSS k v) 0

-- | Amortized \(O(\log N)\). Modifies a kth node's value.
{-# INLINE modifySS #-}
modifySS :: (HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> (v -> v) -> Int -> m ()
modifySS SplaySeq {..} f k = do
  GM.unsafeModifyM rootSS (modifyRSS rawSS f k) 0

-- | Amortized \(O(\log N)\). Exchanges a kth node's value.
{-# INLINE exchangeSS #-}
exchangeSS :: (HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> Int -> v -> m v
...
{-# INLINE foldSS #-}
foldSS :: (HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> Int -> Int -> m v
...
{-# INLINE foldAllSS #-}
foldAllSS :: (HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> m v
...
-- | Amortized \(O(\log N)\).
{-# INLINE sactSS #-}
sactSS :: (HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> Int -> Int -> a -> m ()
...
{-# INLINE reverseSS #-}
reverseSS :: (HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> Int -> Int -> m ()
reverseSS SplaySeq {..} l r = do
  GM.unsafeModifyM rootSS (reverseRSS rawSS l r) 0

-- | Amortized \(O(\log N)\). Inserts a node at @k@.
{-# INLINE insertSS #-}
insertSS :: (HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> Int -> v -> m ()
insertSS SplaySeq {..} k v = do
  GM.unsafeModifyM rootSS (insertRSS rawSS k v) 0

-- | Amortized \(O(\log N)\). Deletes a node at @k@.
{-# INLINE deleteSS #-}
deleteSS :: (HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> Int -> m ()
...
{-# INLINE bisectLSS #-}
bisectLSS :: (Show v, HasCallStack, PrimMonad m, Monoid v, U.Unbox v, Monoid a, U.Unbox a, Eq a) => SplaySeq (PrimState m) v a -> (v -> Bool) -> m (Maybe SplayIndex)
