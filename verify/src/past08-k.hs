...
solve = do
  ...
  let !es3 = (`U.imap` (vecIV mat)) $ \i acc ->
        let (!y, !x) = i `divMod` w
            !dw = dys U.! y + dxs U.! x
         in if dw <= 0 || acc == '0'
              then -- REMARK: Be sure to NOT remove the edge, or else the min cost calculation fails
              -- to calculate the best result
                (y0 + y, x0 + x, 1, 0)
              else (y0 + y, x0 + x, 1, dw)

  let (Max !delta, !_) = note "delta" $ maxCostFlow nVerts src sink targetFlow $ es1 U.++ es2 U.++ es3
  printBSB $ s0 + delta
