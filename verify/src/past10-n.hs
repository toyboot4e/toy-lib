...
solve = do
  ...
  xs <- intsU'
  -- TODO: why this workd??
  let !ps = U.accumulate (+) (U.replicate m (modInt 0)) $ U.map ((,modInt 1) . pred) xs
  let !qs = U.accumulate (+) (U.replicate m (modInt 0)) $ U.map ((,modInt 1) . (m -)) xs
  let !res = convoluteMod ps qs
