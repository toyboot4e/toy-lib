  ...
  testGroup
    ...
    [ QC.testProperty "slide-stack" $ QCM.monadicIO $ do
        ...
        U.forM_ qs $ \case
          (0, !a, !b) -> do
            -- push back
            QCM.run $ Q.pushBack buf . Dual $ Affine1 (a, b)
            QCM.run $ pushBackSSF window . Dual $ Affine1 (a, b)
          (1, !_, !_) -> do
            -- pop froot
            len <- QCM.run $ Q.length buf
            when (len > 0) $ do
              QCM.run $ Q.popFront_ buf
              QCM.run $ popFrontSSF window
          (2, !x, !_) -> do
            -- fold
            expected <- (`sact` x) . getDual <$> QCM.run (U.foldl' (<>) mempty <$> Q.unsafeFreeze buf)
      ...
      QC.testProperty "slide-deque" $ QCM.monadicIO $ do
        ...
        buf <- QCM.run $ Q.newDeque q
        window <- QCM.run $ newDSF q

        U.forM_ qs $ \case
          (0, !a, !b) -> do
            -- push front
            QCM.run $ Q.pushFront buf . Dual $ Affine1 (a, b)
