...
#else
dbg :: Show a => a -> () ; dbg _ = () ; dbgAssert :: Bool -> a -> a ; dbgAssert = const id ; dbgS :: String -> () ; dbgS _ = () ; dbgId :: Show a => a -> a ; dbgId = id ; note :: (Show s, Show a) => s -> a -> a ; note _ !x = x ; dbgSM :: (Monad m) => m String -> m () ; dbgSM _ = return () ;
#endif
